...
int func (integer ndim, const double *u, const integer *icp, const double *par, integer ijac, double *f, double *dfdu, double *dfdp) {
  f[0]= (35*h*(-v + 55)*pow(-0.1*v - 3.5, 3)/(pow(-1 + 0.0301973834223185*exp(-0.1*v), 3)*pow(4*exp(-1.0L/18.0L*v - 10.0L/3.0L) + (-0.1*v - 3.5)/(-1 + 0.0301973834223185*exp(-0.1*v)), 3)) + 9*pow(n, 4)*(-v - 90) - 0.1*v - 6.5 + I)/Cm;
  f[1]= -0.625*n*exp(-1.0L/80.0L*v - 11.0L/20.0L) + (-n + 1)*(-0.05*v - 1.7)/(-1 + 0.0333732699603261*exp(-0.1*v));
  f[2]= -5*h/(1 + 0.0608100626252179*exp(-0.1*v)) + (-0.35*h + 0.35)*exp(-1.0L/20.0L*v - 29.0L/10.0L);
...
int bcnd (integer ndim, const double *par, const integer *icp, integer nbc, const double *u0, const double *u1, integer ijac, double *fb, double *dbc) {
  ...
  fb[1]= n_left - n_right;
  fb[2]= h_left - h_right;
  fb[3]= (35*h_left*(-v_left + 55)*pow(-0.1*v_left - 3.5, 3)/(pow(-1 + 0.0301973834223185*exp(-0.1*v_left), 3)*pow(4*exp(-1.0L/18.0L*v_left - 10.0L/3.0L) + (-0.1*v_left - 3.5)/(-1 + 0.0301973834223185*exp(-0.1*v_left)), 3)) + 9*pow(n_left, 4)*(-v_left - 90) - 0.1*v_left - 6.5 + I)/Cm;
  return 0;
}
