...
def find_ini_brian():
    ...
    varrhs2 = [(i,sympy.S(j).subs(baseunits2))
    ...
    return unames, pnames, inivals
#autobifpar['Cm'] = 1

################ CONT FP & LC ############################

ini = [float(getattr(states,j)[0][-1]) for j in var]
ini[0] *= 1000
#autobifpar['Cm'] = 0.015
pdb.set_trace()


r1_fwd= auto.run(([float(getattr(states,j)[0][-1]) for j in var]), e='tm_new',
    c='tm_new', parnames= pnames_wrap, unames=unames_wrap,
...
r1_fwd= auto.run(ini, e='tm_new',
    ...
    ICP=['I'], ISP=1,ILP=1, SP=['LP','HB','BP'],
    PAR=autobifpar, ITNW=17, NWTN=13, NMX = 130000,NPR=130000, #NMX=113500, NPR=5000,
    DS=1e-2, DSMAX=1e-3, #STOP=['HB1'],
    UZSTOP= {})


#pdb.set_trace()
r1_bwd= auto.run([float(getattr(states,j)[0][-1]) for j in var], e='tm_new',
    c='tm_new', parnames= pnames_wrap, unames=unames_wrap,
...
pdb.set_trace()
direction = -1
runBranch2 = runBranch1 + auto.run(s1HB, e='tm_new', c='tm_new',
                 parnames=pnames_wrap, unames=unames_wrap,
                 ICP=['I','Cm'], #IPS = 1
                 ...
                 ITNW=17, NWTN=13, NMX=30000, NPR=10000, #Not sure if needed: PAR=autobifpar,
                 DS=direction*1e-3, DSMAX=1e-3, #IID = 5, RL1=0.58, EPSL = 1e+1, EPSU = 1e+1, EPSS = 10, NTST = 500,
                 UZSTOP= {}
                 )
runBranch2 += r1_fwd
auto.save(runBranch2, 'rb2')
pdb.set_trace()
