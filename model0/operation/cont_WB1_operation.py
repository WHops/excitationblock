...
p = {"simfile"  : "sim_DNap_2015-12-14_16:55:14.603625.shv",
     ...
     "workdir"  : os.getenv("HOME") + "/Uni/CNS/3.Semester/schreiberlab/excitationblock/model0/",
     "contdir"  : os.getenv("HOME") + "/Uni/CNS/3.Semester/schreiberlab/excitationblock/model0/operation",
     "dt"       : "0.05*ms",
     "bifpar"   : {
       "I" : ["0.* uA/cm2"],
...
os.chdir(p["contdir"])
unames_wrap,pnames_wrap= autoutils.writeFP('tm3',
...
#ini = [-0.06401756490965044, 0.08907801030851664, 0.780791551001918]
#autobifpar = {}

r1_fwd= auto.run([float(getattr(states,j)[0][-1]) for j in var], e='tm3',
    c='tm3', #parnames= pnames_wrap, unames=unames_wrap,
    ICP=['I'], ISP=1,ILP=1, SP=['LP','HB','BP'],
    ITNW=17, NWTN=13, NMX = 1300000,NPR=1, #NMX=113500, NPR=5000, #    PAR=autobifpar,
    DS=1e-6, DSMAX=1e-5, #STOP=['HB1'],
    IID = 3,
    UZSTOP= {})
r1_bwd= auto.run([float(getattr(states,j)[0][-1]) for j in var], e='tm3',
    c='tm3', parnames= pnames_wrap, unames=unames_wrap,
...
r1_period = auto.run(s1HB, e='tm3', c='tm3',
...
pdb.set_trace()
direction = 1
runBranch1 = auto.run(s3HB, e='tm3', c='tm3',
                 parnames=pnames_wrap, unames=unames_wrap,
                 ICP=['I','Cm'], #IPS = 1
...
runBranch2 = auto.run(rb1EP, e='tm3', c='tm3',
