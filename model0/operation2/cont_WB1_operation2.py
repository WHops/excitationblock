...
p = {"simfile"  : "sim_DNap_2015-12-14_16:55:14.603625.shv",
     "modfile"  : "data/wangBuzsaki.json",      #This is our model
     "contfile" : "data/cont_WB.json",   #This is what brian produces and hands to auto
     "workdir"  : os.getenv("HOME") + "/Uni/CNS/3.Semester/schreiberlab/excitationblock/model0/operation2",
     "contdir"  : os.getenv("HOME") + "/Uni/CNS/3.Semester/schreiberlab/excitationblock/model0/operation2",
     "dt"       : "0.05*ms",
     "bifpar"   : {
       "I" : ["0.* uA/cm2"],
...
unames,pnames= autoutils.writeFP('naktest',
...
#autobifpar['Cm'] = 0.015
r1_fwd= auto.run([float(getattr(states,j)[0][-1]) for j in var], e='nak',
    c='nak', parnames= pnames, unames=unames,
    ICP=['I','Cm','period'], ISP=1,ILP=1, SP=['LP','HB','BP'],
    ITNW=17, NWTN=13, NMX = 13000,NPR=1300, #NMX=113500, NPR=5000,
...
r1_fwd= auto.run([float(getattr(states,j)[0][-1]) for j in var], e='nak',
    c='nak', parnames= pnames, unames=unames,
    ICP=['I','Cm','period'], ISP=1,ILP=1, SP=['LP','HB','BP'],
    PAR=autobifpar, ITNW=17, NWTN=13, NMX = 130000,NPR=130000, #NMX=113500, NPR=5000,
    DS=1e-6, DSMAX=1e-5, #STOP=['HB1'],
    IID = 3,
    UZSTOP= {})
r1_bwd= auto.run([float(getattr(states,j)[0][-1]) for j in var], e='nak',
    c='nak', parnames= pnames_wrap, unames=unames_wrap,
    ICP=['I','Cm','period'], ISP=1,ILP=1, SP=['LP','HB','BP'],
    PAR=autobifpar, ITNW=17, NWTN=13, NMX = 30000, NPR=30000,#NMX=113500, NPR=5000,
    DS=-1e-6, DSMAX=1e-5, #STOP=['HB1'],
    IID = 3,
    UZSTOP= {})
...
auto.save(r1_withperiod,'r1_withperiod')

s1HB_final = r1_withperiod.getLabel('HB')[0]


pdb.set_trace()

#--------------Now: we refine the HB ------------------------

r2= auto.run(s1HB_final, e='nak',
    c='nak', parnames= pnames_wrap, unames=unames_wrap,
    ICP=['I','Cm','period'], ISP=1,ILP=1, SP=['LP','HB','BP'], ITNW=17, NWTN=13, NMX=1000000, NPR=1000000,
    DS=-1e-10, DSMAX=1e-10, STOP=['HB1'],
    IID = 3)

s2HB = r2.getLabel('HB')[0]

r3= auto.run(s2HB, e='nak',
    c='nak', parnames= pnames_wrap, unames=unames_wrap,
    ICP=['I','Cm','period'], ISP=1,ILP=1, SP=['LP','HB','BP'], ITNW=17, NWTN=13, NMX=1000000, NPR=1000000,
    DS=1e-12, DSMAX=1e-12, STOP=['HB1'],
    IID = 3)
...
pdb.set_trace()
direction = 1
runBranch1 = auto.run(s1HB, e='nak', c='nak',
                 parnames=pnames, unames=unames,
                 ICP=['I','Cm'], #IPS = 1
                 ...
                 ITNW=17, NWTN=13, NMX=5000, NPR=500, #Not sure if needed: PAR=autobifpar,
                 DS=direction*1e-2, DSMAX=1e-2, #IID = 5, RL1=0.58, EPSL = 1e+1, EPSU = 1e+1, EPSS = 10, NTST = 500,
                 UZSTOP= {}
                 )
...
runBranch2 = auto.run(rb1EP, e='tm', c='tm',
                 parnames=pnames_wrap, unames=unames_wrap, TY='HB',
                 ICP=['Iadd','I','period', 'Iadd'], #IPS = 1
                 ISP=2,ILP=1,# SP=['LP','HB','BP'],   # ISP: Bifurcation detection; 0=off, 1=BP(FP), 3=BP(PO,BVP), 2=all | ILP: Fold detection; 1=on, 0=off
                 ISW=2, # ISW: Branch switching; 1=normal, -1=switch branch (BP, HB, PD), 2=switch to two-parameter continuation (LP, BP, HB, TR), 3=switch to three-parameter continuation (BP)
